...
function buildTicketAuthMessage(args) {
  const msg = {
    type: "auth",
    ticket: args.ticket,
    shared_id: args.sharedId,
...
var AppoWssClient = class {
  constructor(config) {
    ...
    this.rws = new import_reconnecting_websocket.default(config.url, protocols, rwsOptions);
  ...
  async onOpen() {
    ...
    try {
      ...
      const reason = err instanceof Error ? err.message : this.config.getTicket ? "getTicket_failed" : "getToken_failed";
      const failReason = this.config.getTicket ? "getTicket_failed" : "getToken_failed";
...
function getOrCompile(type) {
  ...
  if (!ajvInstance) {
    ajvInstance = new import_ajv.default({ strict: false, allErrors: true });
